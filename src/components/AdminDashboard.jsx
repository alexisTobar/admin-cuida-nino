                ...
                <div className="flex flex-col items-center text-center">
                  <div className="w-48 h-48 md:w-64 md:h-64 rounded-full border-4 md:border-8 border-blue-600/20 p-1 md:p-2 mb-4 md:mb-8">
                    <div className="w-full h-full rounded-full overflow-hidden bg-slate-800 border-2 border-blue-600">
                      {selectedChild.fotoUrl ? <img src={selectedChild.fotoUrl} className="w-full h-full object-cover" alt="Perfil" /> : <div className="w-full h-full flex items-center justify-center text-slate-700"><Users size={60} /></div>}
                    </div>
                  </div>
                  ...
                  {registrados.filter(n => n.nombre.toLowerCase().includes(filtro.toLowerCase())).map(n => (
                    <tr key={n.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4 text-sm font-black text-white uppercase">
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-4">
                          <button onClick={() => setSelectedChild(n)} className="text-slate-600 hover:text-blue-500 transition-colors"><Eye size={18}/></button>
                          <button onClick={() => setEditChild(n)} className="text-slate-600 hover:text-green-500 transition-colors"><Edit3 size={18}/></button>
...
function CardStat({ label, val, icon }) {
  return (
    <div className="bg-white p-8 md:p-12 flex items-center justify-between group border-r border-white/5 text-black">
      <div>
